...
it, and how much pathwidth affects the end-to-end performance of such solvers has
remained unclear.
...
\item We find that the pathwidth of the order given to the solver is a strong
...
instances; for the remaining $293$ graphs ($414$ instances) the optimal order
...
dramatically: the median pathwidth drops from $32$ for \OrigDecom to $11$ for
\HeurDecom and $10$ for \OptDecom.
...
instances solved by all three orders within the time limit ($210$ instances for
the shortest variant and $208$ for the farthest variant), so that every order
...
%% is itself a key benefit (handoff item 4-4).
...
\begin{table}[t]
  ...
  three orders ($210$ for shortest, $208$ for farthest). Values are geometric
  ...
  \begin{tabular}{llrrrr}
    ...
    \hline
    \multirow{3}{*}{shortest}
      & \OrigDecom & $31$ & $3011$ & $0.183$ & $288{,}715$ \\
      & \HeurDecom & $11$ & $1191$ & $0.078$ & $174{,}463$ \\
      & \OptDecom  & $10$ & $\phantom{0}971$ & $0.072$ & $161{,}442$ \\
    \hline
    \multirow{3}{*}{farthest}
      & \OrigDecom & $31$ & $2805$ & $0.159$ & $307{,}301$ \\
      & \HeurDecom & $11$ & $1157$ & $0.074$ & $188{,}011$ \\
      & \OptDecom  & $10$ & $\phantom{0}948$ & $0.069$ & $169{,}237$ \\
    \hline
  \end{tabular}
\end{table}
...
pathwidth and ZDD size is $0.66$, and that between ZDD size and total solving
time is $0.51$.
...
\begin{table}[t]
  ...
  restricted to the $279$ instances for which all three orders are available; the
...
reach.
To confirm that this is a genuine effect of the heuristic order rather than of
...
%% \HeurDecom solves them in seconds (e.g.\ pathwidth drops $234\to23$ and
%% $140\to17$); a few have unchanged pathwidth under reordering and must be
...
lightweight heuristic decomposition is the order that lets the solver make
progress.
...
instances solved within $30$ minutes, how many each order solves.
...
In the long-length ranges ($\ell>10^{4}$) the three orders solve exactly the
...
Within this population, \OptDecom solves every instance that any order solves.
(On the shortest variant there are in addition $10$ instances with no
reconfiguration sequence, all of which are resolved by all three orders; the
...
\begin{table}[t]
  ...
  Column ``\#'' is the number of such instances solved by at least one order.}
  \label{tab:reconflen}
  \begin{tabular}{llrrrr}
    \hline
...
$\ell\approx2.2\times10^{5}$, all have small pathwidth and are solved cheaply,
with time growing only gently with $\ell$.
The instances that approach the $30$-minute limit, by contrast, have short
sequences but large pathwidth.
Difficulty is therefore governed by pathwidth, not by sequence length: once the
pathwidth is controlled for, solving time grows roughly linearly in $\ell$ (the
...
edges, and pathwidth $5$, whose shortest variant admits a sequence of length
...
\begin{figure}[t]
  ...
  its pathwidth. Solving time stays low for the longest sequences, which all have
  small pathwidth, while the instances near the time limit have short sequences
  but large pathwidth: pathwidth, not sequence length, governs difficulty.}
...
point-biserial correlation between solvability and pathwidth is stronger than
that between solvability and the number of edges ($-0.64$ versus $-0.53$ for
\HeurDecom), confirming that pathwidth is the better predictor.
...
\begin{figure}[t]
  ...
  order, for \OrigDecom (top) and \HeurDecom (bottom), on the shortest variant.
